...
""")
        ...
        main_stats_frame.pack(side=tk.TOP, fill=tk.X)
        
        # Left frame for time and FPS
        left_stats_frame = tk.Frame(main_stats_frame)
        left_stats_frame.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=5, pady=2)

        self.timer_label = tk.Label(left_stats_frame, text="Time Remaining: 00:00:00", anchor="w")
        self.timer_label.pack(anchor="w")

        self.timer_running = False
        self.start_time = None

        self.fps_label = tk.Label(left_stats_frame, text="FPS: 0", anchor="w")
        self.fps_label.pack(anchor="w")
        
        # Right frame for casts and fails
        right_stats_frame = tk.Frame(main_stats_frame)
        right_stats_frame.pack(side=tk.RIGHT, expand=True, fill=tk.X, padx=5, pady=2)
        
        self.casts_label = tk.Label(right_stats_frame, text="Total Casts: 0", anchor="e")
        self.casts_label.pack(anchor="e")
        
        self.fails_label = tk.Label(right_stats_frame, text="Total Fails: 0", anchor="e")
        self.fails_label.pack(anchor="e")
    ...
    def update_fails(self, fails):
        self.total_fails = fails
        self.fails_label.config(text=f"Total Fails: {self.total_fails} ")
...
class FishingAgent:
    ...
    def cast_lure(self):
        ...
        self.casts += 1
        self.root.after(0, self.gui.update_casts, self.casts)
        self.fishing = True
